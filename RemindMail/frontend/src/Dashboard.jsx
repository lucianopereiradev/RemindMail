...
export default function Dashboard({ onLogout }) {
  ...
  async function createReminder() {
    ...
    await fetch(`${API_URL}/reminder`, {
      ...
      body: JSON.stringify({ title, description, remind_at: new Date(date).toISOString(), recurring, recurring_type: recurring ? recurringType : null }),
    });
  ...
  async function saveEdit(id) {
    ...
    await fetch(`${API_URL}/reminder/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify({ title: editTitle, description: editDescription, remind_at: new Date(editDate).toISOString(), recurring: editRecurring, recurring_type: editRecurring ? editRecurringType : null }),
    });
    setEditingId(null);
    loadReminders();
  }
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-6xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex justify-between items-center text-white">
          ...
          <div className="flex gap-2">
            <Button onClick={onLogout} className="bg-white/10 hover:bg-white/20 border border-white/20 gap-1 sm:gap-2 text-xs sm:text-sm px-3 sm:px-4 rounded-full">
              <LogOut size={14} /> Sair
            </Button>
            <Button onClick={() => setShowDeleteConfirm(true)} className="bg-red-500/20 hover:bg-red-500/40 border border-red-400/30 text-red-300 rounded-full text-xs sm:text-sm px-3 sm:px-4">
              Excluir conta
            </Button>
          </div>
        </motion.div>
